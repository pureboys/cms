            ...
            {if $CommentList}
                {foreach $CommentList(key,value)}
                    <tr>
                        ...
                        <td>{@value->state}</td>
                        <td><input type="text" value="{@value->num}" class="text sort" name="states[{@value->id}]"/>
                        </td>
                        <td><a href="comment.php?action=delete&id={@value->id}"
                               onclick="return confirm('你真的要删除这条评论么？') ? true : false">删除</a></td>
                    </tr>
                {/foreach}
            {else}
                <tr>
