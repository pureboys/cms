...
class LinkAction extends Action
{
    ...
    private function state()
    {
        if (isset($_GET['id'])) {
            ...
            if (!$this->_model->getOneLink()) Tool::alertBack('不存在此友情连接！');
            if ($_GET['type'] == 'ok') {
    ...
    private function delete()
    {
        if (isset($_GET['id'])) {
            $this->_model->id = $_GET['id'];
            $this->_model->deleteLink() ? Tool::alertLocation('恭喜你，删除友情连接成功！', PREV_URL) : Tool::alertBack('很遗憾，删除失败！');
        } else {
            Tool::alertBack('非法操作！');
        }
    }
