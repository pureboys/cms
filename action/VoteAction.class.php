...
class VoteAction extends Action
{
    ...
    private function add()
    {
        if (isset($_POST['send'])) {
            $this->setAdd();
            $this->_model->addVote() ? Tool::alertLocation('新增投票主题成功！', '?action=show') : Tool::alertBack('警告：新增投票主题失败');
    ...
    private function addchild()
    {
        if (isset($_POST['send'])) {
            ...
            $this->setAdd();
            $this->_model->addVote() ? Tool::alertLocation('新增投票项目成功！', '?action=showchild&id=' . $this->_model->vid) : Tool::alertBack('警告：新增投票项目失败');
        }
        if (isset($_GET['id'])) {
    ...
    {
        if (Validate::checkNull($_POST['title'])) Tool::alertBack("警告：主题不得为空");
        if (Validate::checkLength($_POST['title'], 2, 'min')) Tool::alertBack("警告：主题不得小于2位");
        if (Validate::checkLength($_POST['title'], 20, 'max')) Tool::alertBack("警告：主题不得大于20位");
    ...
    private function update()
    {
        if (isset($_POST['send'])) {
            ...
            $this->_model->updateVote() ? Tool::alertLocation('投票修改成功！', $_POST['prev_url']) : Tool::alertBack('警告：投票主题修改失败！');
    ...
    private function delete()
    {
        if (isset($_GET['id'])) {
            $this->_model->id = $_GET['id'];
            $this->_model->deleteVote() ? Tool::alertLocation('恭喜你，删除投票成功！', PREV_URL) : Tool::alertBack('很遗憾，删除失败！');
        } else {
            Tool::alertBack('非法操作！');
        }
    }
    ...
    private function state()
    {
        if (isset($_GET['id'])) {
            $this->_model->id = $_GET['id'];
            if (!$this->_model->getOneVote()) Tool::alertBack('不存在此主题！');
            if ($_GET['type'] == 'ok') {
